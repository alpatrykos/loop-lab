    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                float2 warpUv = LoopLabApplyOrbit(uv, loopVector, float2(0.018, -0.012), float2(0.010, 0.021));
                float2 ridgeUv = LoopLabApplyOrbit(uv, loopVector, float2(-0.014, 0.008), float2(0.013, 0.016));
                float terrainSweep = dot(loopVector, float2(0.031, -0.019));
                float bandOrbit = dot(loopVector, float2(0.022, 0.014));

                float warpField = TileFbm(warpUv, frac(phase * 2.0 + 0.11), seed + 3.0) - 0.5;
                float ridgeWarp = TileRidgedFbm(ridgeUv, frac(phase * 3.0 + 0.29), seed + 19.0) - 0.5;
                float bandWarp = (warpField * 0.16 + ridgeWarp * 0.11) * (0.72 + detailBias * 0.24);

                float farHeight = 0.46 + TerrainLayer(uv.x + terrainSweep, 0.18 + bandWarp * 0.24 + bandOrbit * 0.18, frac(phase + 0.09), seed + 31.0, 0.05, 1.08);
                float midHeight = 0.31 + TerrainLayer(uv.x + terrainSweep * 1.4, 0.42 + bandWarp * 0.32 - bandOrbit * 0.16, frac(phase * 2.0 + 0.17), seed + 53.0, 0.07, 1.18);
                float nearHeight = 0.18 + TerrainLayer(uv.x + terrainSweep * 1.8, 0.66 + bandWarp * 0.40 + bandOrbit * 0.22, frac(phase * 3.0 + 0.29), seed + 79.0, 0.09, 1.30);
                ...
                float cloudNoise = TileFbm(
                    LoopLabApplyOrbit(uv, loopVector, float2(0.026, 0.0), float2(-0.014, 0.018)),
                    frac(phase * 2.0 + 0.43),
                    seed + 101.0);
                float cloudRidge = TileRidgedFbm(
                    LoopLabApplyOrbit(uv, loopVector, float2(-0.018, 0.013), float2(0.022, -0.011)),
                    frac(phase * 4.0 + 0.59),
                    seed + 127.0);
                ...
                float valleyFogNoise = TileFbm(
                    LoopLabApplyOrbit(uv, loopVector, float2(0.010, -0.015), float2(0.017, 0.012)),
                    frac(phase * 5.0 + 0.71),
                    seed + 149.0);
                float valleyFog = smoothstep(0.18, 0.74, mirroredY) * (1.0 - nearMask * 0.78) * saturate(0.26 + valleyFogNoise * 0.74);

                float farTexture = TileFbm(
                    LoopLabApplyOrbit(uv, loopVector, float2(0.008, -0.006), float2(0.005, 0.009)),
                    frac(phase + 0.17),
                    seed + 173.0);
                float midTexture = TileRidgedFbm(
                    LoopLabApplyOrbit(uv, loopVector, float2(-0.011, 0.007), float2(0.014, 0.010)),
                    frac(phase * 2.0 + 0.33),
                    seed + 197.0);
                float nearTexture = TileRidgedFbm(
                    LoopLabApplyOrbit(uv, loopVector, float2(0.016, -0.010), float2(-0.009, 0.015)),
                    frac(phase * 3.0 + 0.49),
                    seed + 223.0);
                ...
                float grain = (
                    TileFbm(
                        LoopLabApplyOrbit(uv, loopVector, float2(0.006, -0.004), float2(0.004, 0.007)),
                        frac(phase * 6.0 + 0.83),
                        seed + 251.0) - 0.5) * 0.03;
